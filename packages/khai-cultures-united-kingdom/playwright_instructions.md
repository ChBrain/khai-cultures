...
- Declare `@chbrain/khai-cultures-united-kingdom` as a dependency, then link `@chbrain/khai-cultures-united-kingdom/position_culture_britishness.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `united_kingdom`'s play. You wire to its position,

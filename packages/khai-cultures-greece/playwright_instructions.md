...
- Declare `@chbrain/khai-cultures-greece` as a dependency, then link `@chbrain/khai-cultures-greece/position_culture_ellinikotita.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `greece`'s play. You wire to its position,

...
- Declare `@chbrain/khai-cultures-de-lower-saxony` as a dependency, then link `@chbrain/khai-cultures-de-lower-saxony/position_culture_die_niedersaechsische_kultur.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `de_lower_saxony`'s play. You wire to its position,

...
# Instructions: Es Balearic Islands
...
- Declare `@chbrain/khai-cultures-es-balearic-islands` as a dependency, then link `@chbrain/khai-cultures-es-balearic-islands/position_culture_cultura_balear.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `es_balearic_islands`'s play. You wire to its position,

...
- Declare `@chbrain/khai-cultures-slovakia` as a dependency, then link `@chbrain/khai-cultures-slovakia/position_culture_mlady_narod.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `slovakia`'s play. You wire to its position,

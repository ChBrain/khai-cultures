...
- Declare `@chbrain/khai-cultures-gb-scotland` as a dependency, then link `@chbrain/khai-cultures-gb-scotland/position_culture_scottishness.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `gb_scotland`'s play. You wire to its position,

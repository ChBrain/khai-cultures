...
- Declare `@chbrain/khai-cultures-usa` as a dependency, then link `@chbrain/khai-cultures-usa/position_culture_american_way.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `usa`'s play. You wire to its position,

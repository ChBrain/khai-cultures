...
- Declare `@chbrain/khai-cultures-es-catalonia` as a dependency, then link `@chbrain/khai-cultures-es-catalonia/position_culture_identidad_catalana.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `es_catalonia`'s play. You wire to its position,

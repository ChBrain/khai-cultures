...
- Declare `@chbrain/khai-cultures-france` as a dependency, then link `@chbrain/khai-cultures-france/position_culture_une_certaine_idee_de_la_france.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `france`'s play. You wire to its position,

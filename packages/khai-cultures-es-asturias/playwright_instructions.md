...
- Declare `@chbrain/khai-cultures-es-asturias` as a dependency, then link `@chbrain/khai-cultures-es-asturias/position_culture_cultura_asturiana.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `es_asturias`'s play. You wire to its position,

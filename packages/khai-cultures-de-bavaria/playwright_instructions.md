...
- Declare `@chbrain/khai-cultures-de-bavaria` as a dependency, then link `@chbrain/khai-cultures-de-bavaria/position_culture_bayerische_kultur.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `de_bavaria`'s play. You wire to its position,

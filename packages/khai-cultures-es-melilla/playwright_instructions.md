...
- Declare `@chbrain/khai-cultures-es-melilla` as a dependency, then link `@chbrain/khai-cultures-es-melilla/position_culture_la_melillensidad.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `es_melilla`'s play. You wire to its position,

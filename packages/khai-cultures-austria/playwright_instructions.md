...
- Declare `@chbrain/khai-cultures-austria` as a dependency, then link `@chbrain/khai-cultures-austria/position_culture_oesterreichische_kultur.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `austria`'s play. You wire to its position,

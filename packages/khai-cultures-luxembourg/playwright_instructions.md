...
- Declare `@chbrain/khai-cultures-luxembourg` as a dependency, then link `@chbrain/khai-cultures-luxembourg/position_culture_mir_welle_bleiwe.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `luxembourg`'s play. You wire to its position,

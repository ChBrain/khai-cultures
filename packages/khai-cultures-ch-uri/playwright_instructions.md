...
- Declare `@chbrain/khai-cultures-ch-uri` as a dependency, then link `@chbrain/khai-cultures-ch-uri/position_culture_urner_kultur.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `ch_uri`'s play. You wire to its position,

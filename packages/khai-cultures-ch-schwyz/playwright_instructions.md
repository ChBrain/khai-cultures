...
- Declare `@chbrain/khai-cultures-ch-schwyz` as a dependency, then link `@chbrain/khai-cultures-ch-schwyz/position_culture_schwyzer_kultur.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `ch_schwyz`'s play. You wire to its position,

...
- Declare `@chbrain/khai-cultures-us-pennsylvania` as a dependency, then link `@chbrain/khai-cultures-us-pennsylvania/position_culture_keystone_culture.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `us_pennsylvania`'s play. You wire to its position,

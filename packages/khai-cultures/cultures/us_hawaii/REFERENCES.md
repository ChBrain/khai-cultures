---
updated: "2026-07-06"
---

# Hawaii: References

The provenance behind this culture: the Hofstede source data that tunes the pitch (kept here, never in the prose) and the public-domain historical sources behind the plots, personas, places, and pieces.

## Hofstede source data

The pitch [pitch_hawaii.md](pitch_hawaii.md) is written from Hawaii's profile inflected by its Polynesian settlement, royal kingdom history, US annexation, military installations, and cultural revival, naming no dimension and carrying no number.

| Dimension                     | Hawaii base score (estimated / US proxy) |
| ----------------------------- | ---------------------------------------- |
| Power Distance                | 44                                       |
| Individualism                 | 70                                       |
| Motivation toward Achievement | 50                                       |
| Uncertainty Avoidance         | 40                                       |
| Long Term Orientation         | 28                                       |
| Indulgence                    | 75                                       |

Source: Geert Hofstede, Hofstede Insights (US comparison, adjusted for Hawaii's close community networks, _ʻohana_ collective values, high indulgence in recreation/festive arts, and royal historical legacy).

## Historical sources

The plots, the historic personas they cast, the places, and the pieces are drawn from the public historical record.

| Subject                    | In this culture                                    | Scope                                                                                               |
| -------------------------- | -------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| Polynesian Voyaging        | [plot_01_voyaging.md](plot_01_voyaging.md)         | The settlement of the islands by skilled navigators using double-hulled outrigger canoes.           |
| Overthrow of the Kingdom   | [plot_08_overthrow.md](plot_08_overthrow.md)       | The 1893 coup d'état by sugar planters and US marines against Queen Liliʻuokalani.                  |
| Pearl Harbor Attack (1941) | [plot_09_pearl_harbor.md](plot_09_pearl_harbor.md) | The surprise military bombardment that drew the United States into WWII.                            |
| Pearl Harbor Memorial      | [place_pearl_harbor.md](place_pearl_harbor.md)     | The historic military port and national memorial over the sunken USS Arizona.                       |
| Na Pali Coast              | [place_na_pali.md](place_na_pali.md)               | The secluded coastal valleys on Kauai, representing traditional agricultural and natural isolation. |

The defining personas [Keanu](persona_keanu.md) and [Leilani](persona_leilani.md) are archetypes representing the outrigger canoe builder/navigator and the Hawaiian cultural historian.

Content is CC-BY-NC-SA, code is MIT.

...
The pitch [pitch_ch.md](pitch_ch.md) is written from Switzerland's profile on
...
| Dimension                     | Switzerland score (approx.) |
| ----------------------------- | --------------------------- |
| Power Distance                | 34                          |
| Individualism                 | 68                          |
| Motivation toward Achievement | 70                          |
...
best reviewed by a native speaker; like Italian, it is routed to assisted
verification rather than the local detector.
...
tells about itself. The defining personas [Vreni](persona_vreni.md),
[Marco](persona_marco.md), [Gian](persona_gian.md), [Menga](persona_menga.md),
and [Urs](persona_urs.md) are archetypes, not historical individuals: composites
carrying the country's living tensions across its language regions,
generations, and, for Urs, its private banking sector.

The estimate of 20000 to 25000 people turned back at the border during the war
years, and of roughly 300000 admitted, follows the range given by the
independent Bergier Commission (Unabhängige Expertenkommission Schweiz, Zweiter
Weltkrieg); figures in the literature vary and are cited here as orders of
magnitude, not an exact count.

### The Gian and Marco waivers, retired

Earlier drafts of this culture waived [persona_gian.md](persona_gian.md) and
...
same device the house already uses in `position_*.md` and `plan_*.md` files to
span a founding moment and its living heirs, applied here inside a plot for
the first time. `coverage-waivers.json` is removed with this change.

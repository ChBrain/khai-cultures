...
- Declare `@chbrain/khai-cultures-estonia` as a dependency, then link `@chbrain/khai-cultures-estonia/position_culture_eestlus.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `estonia`'s play. You wire to its position,

...
- Declare `@chbrain/khai-cultures-es-navarre` as a dependency, then link `@chbrain/khai-cultures-es-navarre/position_culture_cultura_navarra.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `es_navarre`'s play. You wire to its position,

...
- Declare `@chbrain/khai-cultures-es-la-rioja` as a dependency, then link `@chbrain/khai-cultures-es-la-rioja/position_culture_la_riojanidad.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `es_la_rioja`'s play. You wire to its position,

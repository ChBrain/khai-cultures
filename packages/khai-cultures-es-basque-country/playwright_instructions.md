...
# Instructions: Es Basque Country
...
- Declare `@chbrain/khai-cultures-es-basque-country` as a dependency, then link `@chbrain/khai-cultures-es-basque-country/position_culture_cultura_vasca.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `es_basque_country`'s play. You wire to its position,

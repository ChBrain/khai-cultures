...
- Declare `@chbrain/khai-cultures-holy-see` as a dependency, then link `@chbrain/khai-cultures-holy-see/position_culture_romanitas.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `holy_see`'s play. You wire to its position,

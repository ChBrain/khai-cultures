...
- Declare `@chbrain/khai-cultures-netherlands` as a dependency, then link `@chbrain/khai-cultures-netherlands/position_culture_nederlandse_cultuur.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `netherlands`'s play. You wire to its position,

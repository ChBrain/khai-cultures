---
khai: play
title: "Neuchâtel"
description: "Neuchâtel: The watchmaking republic that threw off the Prussian crown to forge a secular and innovative identity."
declared: "Neuchâtel"
language: fr
license: CC-BY-NC-SA-4.0
stamp:
  owner: KAI HACKS AI
  version: v0.0.1
  date: "2026-08-31"
---

# Play: Neuchâtel

## Taxonomy
La république horlogère.

## Owner
- Project: khai-cultures

## Arc
Un territoire à la trajectoire politique unique : principauté sous la couronne prussienne tout en étant alliée suisse, refuge de penseurs radicaux et d'anarchistes, berceau brutal de l'industrialisation horlogère, avant de s'affranchir de la monarchie par une révolution idyllique en 1848. Aujourd'hui, c'est une république fière, naviguant avec angoisse entre un héritage mécanique magnifié et une innovation de pointe qui menace de l'effacer.

## Name
La République et Canton de Neuchâtel.

## Stakes
Comment se réinventer quand l'industrie horlogère qui a littéralement dicté la forme de vos villes, le rythme de votre sommeil et la structure de votre âme subit des crises existentielles à l'ère numérique ? La perfection technique peut-elle sauver de l'obsolescence, ou n'est-elle qu'une élégante prison ?

## Estate
- House: packages/khai-cultures/README.md

## Company
- `pitch`: [Neuchâtel](pitch_neuchatel.md)
- `plot_00`: [Le Comté et la Réforme](plot_00_le_comte_et_la_reforme.md)
- `plot_01`: [La Couronne Prussienne](plot_01_la_couronne_prussienne.md)
- `plot_02`: [La Métropole Horlogère](plot_02_la_metropole_horlogere.md)
- `plot_03`: [La Révolution 1848](plot_03_la_revolution_1848.md)
- `plot_99`: [La République Industrielle](plot_99_la_republique_industrielle.md)
- `persona`: [L'Horloger](persona_l_horloger.md)
- `persona`: [Le Républicain](persona_le_republicain.md)
- `piece`: [La Montre Mécanique](piece_la_montre_mecanique.md)
- `piece`: [L'Absinthe](piece_l_absinthe.md)
- `place`: [La Chaux-de-Fonds](place_la_chaux_de_fonds.md)
- `place`: [Le Château de Neuchâtel](place_le_chateau_de_neuchatel.md)
- `position`: [La République](position_culture_la_republique.md)
- `position`: [L'Esprit Innovateur](position_l_esprit_innovateur.md)
- `process`: [La Marche du 1er Mars](process_la_marche_du_1er_mars.md)

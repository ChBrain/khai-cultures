...
- Declare `@chbrain/khai-cultures-ch-graubuenden` as a dependency, then link `@chbrain/khai-cultures-ch-graubuenden/position_culture_buendner_kultur.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `ch_graubuenden`'s play. You wire to its position,

...
- Declare `@chbrain/khai-cultures-hong-kong` as a dependency, then link `@chbrain/khai-cultures-hong-kong/position_culture_hongkonger.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `hong_kong`'s play. You wire to its position,

...
- Declare `@chbrain/khai-cultures-albania` as a dependency, then link `@chbrain/khai-cultures-albania/position_culture_shqiptaria.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `albania`'s play. You wire to its position,

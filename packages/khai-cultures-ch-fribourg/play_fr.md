---
khai: play
title: "Fribourg"
description: "Fribourg: The stubborn Catholic bastion that built an intellectual elite and now bridges the French-German linguistic divide."
declared: "Fribourg"
language: fr
license: CC-BY-NC-SA-4.0
stamp:
  owner: KAI HACKS AI
  version: v0.0.1
  date: "2026-08-31"
---

# Play: Fribourg

## Taxonomy
Le pont catholique sur la Sarine.

## Owner
- Project: khai-cultures

## Arc
L'histoire d'un bastion farouchement indépendant qui est resté l'ancre catholique de l'ouest, a combattu et perdu la guerre du Sonderbund en 1847, mais a riposté intellectuellement en fondant une université pour former une élite conservatrice. Aujourd'hui, il est le pont physique et culturel du "Röstigraben", unissant la Suisse romande et alémanique.

## Name
Le canton de Fribourg.

## Stakes
Comment préserver une identité catholique conservatrice et rurale tout en devenant le pont universitaire bilingue d'une Suisse moderne ?

## Estate
- House: packages/khai-cultures/README.md

## Company
- `pitch`: [Fribourg](pitch_fribourg.md)
- `plot_00`: [La Fondation Zähringen 1157](plot_00_la_fondation_zahringen_1157.md)
- `plot_01`: [Le Bastion Catholique](plot_01_le_bastion_catholique.md)
- `plot_02`: [La Défaite du Sonderbund 1847](plot_02_la_defaite_du_sonderbund_1847.md)
- `plot_03`: [L'Université 1889](plot_03_l_universite_1889.md)
- `plot_99`: [Le Pont des Langues](plot_99_le_pont_des_langues.md)
- `persona`: [Le Paysan Gruérien](persona_le_paysan_gruerien.md)
- `persona`: [L'Étudiant Catholique](persona_l_etudiant_catholique.md)
- `piece`: [Le Gruyère](piece_le_gruyere.md)
- `piece`: [Le Bilinguisme](piece_le_bilinguisme.md)
- `place`: [La Cathédrale St-Nicolas](place_la_cathedrale_st_nicolas.md)
- `place`: [La Sarine](place_la_sarine.md)
- `position`: [Le Bastion Catholique](position_culture_le_bastion_catholique.md)
- `position`: [Le Pont des Langues](position_le_pont_des_langues.md)
- `process`: [La Poya](process_la_poya.md)


...
- Declare `@chbrain/khai-cultures-de-brandenburg` as a dependency, then link `@chbrain/khai-cultures-de-brandenburg/position_culture_brandenburgische_kultur.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `de_brandenburg`'s play. You wire to its position,

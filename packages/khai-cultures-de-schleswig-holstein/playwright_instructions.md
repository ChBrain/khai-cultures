...
# Instructions: De Schleswig Holstein
...
- Declare `@chbrain/khai-cultures-de-schleswig-holstein` as a dependency, then link `@chbrain/khai-cultures-de-schleswig-holstein/position_culture_schleswig_holsteinische_kultur.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `de_schleswig_holstein`'s play. You wire to its position,

...
- Declare `@chbrain/khai-cultures-finland` as a dependency, then link `@chbrain/khai-cultures-finland/position_culture_suomalaisuus.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `finland`'s play. You wire to its position,

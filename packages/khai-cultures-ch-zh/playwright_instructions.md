...
- Declare `@chbrain/khai-cultures-ch-zh` as a dependency, then link `@chbrain/khai-cultures-ch-zh/position_culture_zuercher_kultur.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `ch_zh`'s play. You wire to its position,

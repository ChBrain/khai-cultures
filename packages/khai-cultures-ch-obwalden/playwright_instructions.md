...
- Declare `@chbrain/khai-cultures-ch-obwalden` as a dependency, then link `@chbrain/khai-cultures-ch-obwalden/position_culture_obwaldner_kultur.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `ch_obwalden`'s play. You wire to its position,

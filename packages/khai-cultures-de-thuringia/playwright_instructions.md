...
- Declare `@chbrain/khai-cultures-de-thuringia` as a dependency, then link `@chbrain/khai-cultures-de-thuringia/position_culture_thueringische_kultur.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `de_thuringia`'s play. You wire to its position,

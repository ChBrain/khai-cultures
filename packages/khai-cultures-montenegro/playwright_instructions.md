...
- Declare `@chbrain/khai-cultures-montenegro` as a dependency, then link `@chbrain/khai-cultures-montenegro/position_culture_crnogorstvo.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `montenegro`'s play. You wire to its position,

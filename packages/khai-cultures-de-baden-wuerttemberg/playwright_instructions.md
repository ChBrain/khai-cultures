...
# Instructions: De Baden Wuerttemberg
...
- Declare `@chbrain/khai-cultures-de-baden-wuerttemberg` as a dependency, then link `@chbrain/khai-cultures-de-baden-wuerttemberg/position_culture_baden_wuerttembergische_kultur.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `de_baden_wuerttemberg`'s play. You wire to its position,

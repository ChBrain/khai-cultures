...
- Declare `@chbrain/khai-cultures-us-virginia` as a dependency, then link `@chbrain/khai-cultures-us-virginia/position_culture_old_dominion.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `us_virginia`'s play. You wire to its position,

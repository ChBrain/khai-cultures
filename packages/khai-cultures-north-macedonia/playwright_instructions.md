...
- Declare `@chbrain/khai-cultures-north-macedonia` as a dependency, then link `@chbrain/khai-cultures-north-macedonia/position_culture_makedonstvo.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `north_macedonia`'s play. You wire to its position,

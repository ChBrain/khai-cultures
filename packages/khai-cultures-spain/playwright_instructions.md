...
- Declare `@chbrain/khai-cultures-spain` as a dependency, then link `@chbrain/khai-cultures-spain/position_culture_espanolidad.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `spain`'s play. You wire to its position,

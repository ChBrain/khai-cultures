...
# Instructions: Es Valencian Community
...
- Declare `@chbrain/khai-cultures-es-valencian-community` as a dependency, then link `@chbrain/khai-cultures-es-valencian-community/position_culture_cultura_valenciana.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `es_valencian_community`'s play. You wire to its position,

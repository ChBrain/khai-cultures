...
- Declare `@chbrain/khai-cultures-germany` as a dependency, then link `@chbrain/khai-cultures-germany/position_culture_deutsche_kultur.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `germany`'s play. You wire to its position,

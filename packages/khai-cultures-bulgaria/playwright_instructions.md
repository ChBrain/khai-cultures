...
- Declare `@chbrain/khai-cultures-bulgaria` as a dependency, then link `@chbrain/khai-cultures-bulgaria/position_culture_balgarshtina.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `bulgaria`'s play. You wire to its position,

...
- Declare `@chbrain/khai-cultures-iceland` as a dependency, then link `@chbrain/khai-cultures-iceland/position_culture_islensk_menning.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `iceland`'s play. You wire to its position,

...
- Declare `@chbrain/khai-cultures-es-madrid` as a dependency, then link `@chbrain/khai-cultures-es-madrid/position_culture_madrilenidad.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `es_madrid`'s play. You wire to its position,

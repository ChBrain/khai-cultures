...
- Declare `@chbrain/khai-cultures-switzerland` as a dependency, then link `@chbrain/khai-cultures-switzerland/position_culture_schweizer_kultur.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `switzerland`'s play. You wire to its position,

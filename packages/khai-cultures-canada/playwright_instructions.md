...
- Declare `@chbrain/khai-cultures-canada` as a dependency, then link `@chbrain/khai-cultures-canada/position_culture_canadian_culture.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `canada`'s play. You wire to its position,

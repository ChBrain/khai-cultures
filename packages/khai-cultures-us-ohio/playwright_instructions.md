...
- Declare `@chbrain/khai-cultures-us-ohio` as a dependency, then link `@chbrain/khai-cultures-us-ohio/position_culture_ohio_culture.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `us_ohio`'s play. You wire to its position,

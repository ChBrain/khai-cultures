...
- Declare `@chbrain/khai-cultures-es-galicia` as a dependency, then link `@chbrain/khai-cultures-es-galicia/position_culture_identidad_gallega.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `es_galicia`'s play. You wire to its position,

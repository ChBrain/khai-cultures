...
- Declare `@chbrain/khai-cultures-malta` as a dependency, then link `@chbrain/khai-cultures-malta/position_culture_zghira_imma_shiha.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `malta`'s play. You wire to its position,

...
# Instructions: Es Castile And Leon
...
- Declare `@chbrain/khai-cultures-es-castile-and-leon` as a dependency, then link `@chbrain/khai-cultures-es-castile-and-leon/position_culture_cultura_castellanoleonesa.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `es_castile_and_leon`'s play. You wire to its position,

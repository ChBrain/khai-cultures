...
# Instructions: De Rhineland Palatinate
...
- Declare `@chbrain/khai-cultures-de-rhineland-palatinate` as a dependency, then link `@chbrain/khai-cultures-de-rhineland-palatinate/position_culture_rheinland_pfaelzische_kultur.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `de_rhineland_palatinate`'s play. You wire to its position,

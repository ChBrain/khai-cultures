...
- Declare `@chbrain/khai-cultures-croatia` as a dependency, then link `@chbrain/khai-cultures-croatia/position_culture_hrvatstvo.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `croatia`'s play. You wire to its position,

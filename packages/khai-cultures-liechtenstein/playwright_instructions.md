...
- Declare `@chbrain/khai-cultures-liechtenstein` as a dependency, then link `@chbrain/khai-cultures-liechtenstein/position_culture_die_liechtensteinische_kultur.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `liechtenstein`'s play. You wire to its position,

...
- Declare `@chbrain/khai-cultures-hungary` as a dependency, then link `@chbrain/khai-cultures-hungary/position_culture_magyarsag.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `hungary`'s play. You wire to its position,

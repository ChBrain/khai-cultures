---
khai: play
title: "St. Gallen"
description: "St. Gallen: The artificial ring canton that evolved from a monastic superpower to a booming, vulnerable global textile empire."
declared: "St. Gallen"
language: de
license: CC-BY-NC-SA-4.0
stamp:
  owner: KAI HACKS AI
  version: v0.0.1
  date: "2026-08-31"
---

# Play: St. Gallen

## Taxonomy
Das Flickwerk der Exportweltmeister.

## Owner
- Project: khai-cultures

## Arc
Die tragische und funkelnde Geschichte eines zerrissenen Raums. Ein tausendjähriger, allmächtiger katholischer Klosterstaat und eine kalte, protestantische Handelsstadt bekämpften sich bis aufs Blut, bevor Napoleons Feder sie 1803 in das künstliche Korsett eines „Ringkantons“ presste. Erst im späten 19. Jahrhundert fand dieses Frankenstein-Gebilde eine flüchtige, delirierende Seele durch den globalen Goldrausch der Stickerei – bis der grausame Kollaps der 1920er Jahre die Stadt in den Abgrund riss und einen traumatisierten, pragmatischen und dezentralen Kanton zurückliess.

## Name
Kanton St. Gallen.

## Stakes
Wie überlebt eine künstlich erzwungene Schicksalsgemeinschaft, wenn ihr einziger gemeinsamer Nenner – der wahnwitzige, weltumspannende Reichtum – in Staub zerfallen ist und nur das Misstrauen der Talschaften übrigbleibt?

## Estate
- House: packages/khai-cultures/README.md

## Company
- `pitch`: [St. Gallen](pitch_st_gallen.md)
- `plot_00`: [Die Fürstabtei](plot_00_die_fuerstabtei.md)
- `plot_01`: [Die Gespaltene Stadt](plot_01_die_gespaltene_stadt.md)
- `plot_02`: [Napoleons Flickenteppich 1803](plot_02_napoleons_flickenteppich_1803.md)
- `plot_03`: [Die Stickereiblüte](plot_03_die_stickereibluete.md)
- `plot_04`: [Der Stickereikollaps](plot_04_der_stickereikollaps.md)
- `plot_99`: [Der Ringkanton](plot_99_der_ringkanton.md)
- `persona`: [Der Abt](persona_der_abt.md)
- `persona`: [Der Stickereibaron](persona_der_stickereibaron.md)
- `piece`: [Die St. Galler Spitze](piece_die_st_galler_spitze.md)
- `piece`: [Die Olma-Bratwurst](piece_die_olma_bratwurst.md)
- `place`: [Der Stiftsbezirk](place_der_stiftsbezirk.md)
- `place`: [Das Textilmuseum](place_das_textilmuseum.md)
- `position`: [Der Ringkanton](position_culture_der_ringkanton.md)
- `position`: [Die Exportwirtschaft](position_die_exportwirtschaft.md)
- `process`: [Die Olma](process_die_olma.md)

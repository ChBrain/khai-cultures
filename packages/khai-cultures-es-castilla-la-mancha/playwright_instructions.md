...
# Instructions: Es Castilla La Mancha
...
- Declare `@chbrain/khai-cultures-es-castilla-la-mancha` as a dependency, then link `@chbrain/khai-cultures-es-castilla-la-mancha/position_culture_cultura_manchega.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `es_castilla_la_mancha`'s play. You wire to its position,

...
- Declare `@chbrain/khai-cultures-ch-bern` as a dependency, then link `@chbrain/khai-cultures-ch-bern/position_culture_berner_kultur.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `ch_bern`'s play. You wire to its position,

...
- Declare `@chbrain/khai-cultures-romania` as a dependency, then link `@chbrain/khai-cultures-romania/position_culture_romanitatea.md` from the persona who belongs to it. A relative path resolves in a working tree
...
- This package is one culture staged as a full khai play: its own plots, its own
  cast, its own pitch, anchored by `romania`'s play. You wire to its position,

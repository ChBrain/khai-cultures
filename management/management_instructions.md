...
- **The language engine.** A persona uses language through the ladder rooted at
  [Using Language](process_using_language.md): five channels - speaking and
  writing outward, hearing and reading inward, thinking underneath - each held
  per language at its true width, from the tongue gripped as a foreign object to
  the tongue that needs no grip because it is already the body. Every persona
  reads **per language, per channel, at its width**, and the prose enacts the
  **gap** where meaning is lost at the limits of a tongue: shown in the first
  beat, never named.
- **Communication is emergent,** read off two personas' reads and never authored
  pairwise. Whether meaning crosses, and how much, resolves by a ladder:
  1. Same mother tongue and variety -> it crosses whole, culture and all.
  2. Same language, different variety -> the words cross, idiom and connotation
     slip.
  3. Only the worn common tongue -> the transaction crosses, the culture does
     not.
  4. No shared tongue -> a proxy relays it twice-worn, stripped at each hop, and
     the proxy holds the power.
  5. No shared tongue and no proxy -> silence; it does not cross.
- **A language variety is a `position`** (Has / Orders / Loses / Drives), held
  and shared by a culture's personas; `pitch` stays reserved for the single
  Hofstede tone, never used for varieties.
